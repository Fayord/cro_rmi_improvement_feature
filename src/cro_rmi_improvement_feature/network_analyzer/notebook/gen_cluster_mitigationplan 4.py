...
def generate_prompt(cluster, G, nodes):
    ...
    for node in cluster:
        ...
        label = next((item['data']['label'] for item in nodes if item['data']['id'] == node), None)
        prompt += f"- {node}: {label}\n"
        # print(label)
        # print(node)
...
def filter_none_edges(edges, slider_value):
    new_edges = []
    for edge in edges:
        if edge['interdependency_type'] == 'None':
        # if edge["direction"] is not None:
        # if edge["direction"] != "none":
            if edge["cosine_similarity"] >= slider_value:
                new_edges.append(edge)
...
def find_graph_properties(data_path_dict, CLUSTER_METHOD = 'leiden'): # sink/source/central nodes; clusters
    all_data_list = []
    ...
    # all_data_df.replace("Missing value", np.nan, inplace=True)
    # all_data_df.head()
...
def find_graph_properties_newpickle(data_path, company_name, CLUSTER_METHOD = 'leiden'): # sink/source/central nodes; clusters
...
def find_clusters_from_graph(data_path, company_name, CLUSTER_METHOD = 'leiden', graph_dir = 'directed'): # sink/source/central nodes; clusters
    ...
    if CLUSTER_METHOD == 'leiden':
        ...
        # partition = leidenalg.find_partition(G_ig, leidenalg.CPMVertexPartition) # doesn't seem to work with directed graph
        partition = leidenalg.find_partition(G_ig, leidenalg.RBERVertexPartition)
        # partition = leidenalg.find_partition(G_ig, leidenalg.RBERVertexPartition, resolution_parameter=2.0, weights="weight" )
        # partition = leidenalg.find_partition(G_ig, leidenalg.ModularityVertexPartition) # for non-directed
...
def top_n_with_strict_limit(df, column, n=5):
    ...
    top_unique_values = df[column].drop_duplicates().sort_values(ascending=False).tolist()
...
# Always keep all rows with the highest score, even if it exceeds N.
# After that, include additional values only if the total row count stays ≤ N.
# If adding the next value would push the total over N, do not include it.
def top_n_strict_with_priority_on_highest(df, column, n=3):
    df_sorted = df.sort_values(by=column, ascending=False)
    counts = df_sorted[column].value_counts().sort_index(ascending=False)
...
def top_n_with_threshold(df, column, n=3, threshold=0):
    """
    Keep:
...
class control_action_with_related_risk(BaseModel):
    action: str = Field(...,description="Action, policy, or system to be put in place.")
    risks_covered: str = Field(...,description="List the risk codes or descriptions impacted directly or indirectly by this action")
    duration: str = Field(...,description="Short-term (0–6 months), medium (6–12 months), or long-term (1 year+). Explain the rationale.") # TODO: change to literal?

class risk_addressed_and_target(BaseModel):
    risk_addressed: str = Field(...,description="risk code or descriptions impacted directly or indirectly by this plan")
    target_risk_likelihood: str = Field(...,description="target risk likelihood after the plan completion: Rare, Unlikely, Moderate, High, or Almost Certain")
    target_risk_impact: str = Field(...,description="target risk impact after the plan completion: Very Low, Minor, Medium, Major, or Critical")

class DetailedActionPlanCluster(BaseModel): # TODO: modify for control_cluster plan
    cluster_plan_name: str = Field(...,description="A clear and concise title that reflects the nature or focus of the response (e.g., “Agile Innovation Pipeline” or “Talent Resilience Program”)")
    objective: str = Field(...,description="A brief explanation of the systemic challenge the plan addresses and why this approach is selected (e.g., “To reduce exposure to rapidly evolving competition and delayed innovation by enhancing adaptive capability.”)")
    priority_level: str = Field(...,description="Plan's priority level: Low, Medium, or High")
    key_impact: str = Field(...,description="Describe how the intervention diffuses through the network of dependencies to reduce overall exposure.")
    ...
    risks_addressed: List[risk_addressed_and_target] = Field(...,description="List the risk codes or descriptions impacted directly or indirectly by this plan")
    ...
    control_actions: List[control_action_with_related_risk] = Field(...,description="A set of coordinated actions, policies, or systems to be put in place. Should include both technical/process and organizational levers.")
    # cascading_effect: str = Field(...,description=" Explain how addressing direct risks reduces dependent risks")
    cascading_effect: str = Field(...,description="Explain clearly how all risks listed in `risks_addressed` are causally or indirectly connected through the dependency graph. Show the logical or temporal sequence of how controlling one or more risks leads to the reduction or prevention of the others.")
    metrics: List[str] = Field(...,description="Quantitative or qualitative signals that track effectiveness of the controls.")
    responsible_party: str = Field(...,description="Who is accountable for the implementation and monitoring (e.g., Innovation Team, HR, Product Operations)")
    timeline: str = Field(...,description="Short-term (0–6 months), medium (6–12 months), or long-term (1 year+). Can be phased.") # TODO: change to literal? Literal["Short-term", "Medium-term", "Long-term"]
    expected_cost: str = Field(...,description="Plan cost in THB, $, and CNY")
    expected_financial_benefit: str = Field(...,description="Financial benefit when implemented the plan in THB, $, and CNY")
    expected_nonfinancial_benefit: str = Field(...,description="Non-financial benefits when implemented the plan")
...
class RiskMitigationActionClusterPlan(BaseModel): # TODO: modify for control_cluster plan
