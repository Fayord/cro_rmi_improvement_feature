    ...
    for company in companys:
        ...
        for node_edge in snapshot_data_list:
            if node_edge["data"].get("source") is not None:
                ...
                node_data_list.append(node_edge["data"])
        # random only 20% of edge_data_list
        edge_data_list = random.sample(edge_data_list, int(len(edge_data_list) * 0.2))
        node_data_df = pd.DataFrame(node_data_list)
        ...
        for edge_data in edge_data_list:
            ...
            # Convert to row data for Excel
            # Create row data

            source_risk_data = f"risk_name: {source_risk_name} \n risk_desc: {risk_a['risk_desc']} \n rootcause: {risk_a['rootcause']} \n process: {risk_a['process']}"
            target_risk_data = f"risk_name: {target_risk_name} \n risk_desc: {risk_b['risk_desc']} \n rootcause: {risk_b['rootcause']} \n process: {risk_b['process']}"
            row_data = {
                "source_risk": source_risk_name,
                "target_risk": target_risk_name,
            ...
            new_edge_data_dict[len(new_edge_data_dict)] = row_data

    # Convert dictionary to DataFrame and save as Excel
    new_edge_relationship_path = real_data_path.replace(
        ".pkl", "-edge_relationship_for_label.xlsx"
    )
    print(f"relation_count: {relation_count}")
    # Convert dictionary to DataFrame
    df = pd.DataFrame.from_dict(new_edge_data_dict, orient="index")

    # Save to Excel
    df.to_excel(new_edge_relationship_path, index=False)
    print(f"Saved edge relationship data to Excel: {new_edge_relationship_path}")
...
if __name__ == "__main__":
    ...
    print(f"Total Cost (Baht): {cb.total_cost * 35}")
